...
const endOfDayISO = (d: Date) =>
  new Date(Date.UTC(d.getFullYear(), d.getMonth(), d.getDate(), 23, 59, 59)).toISOString().slice(0, 10);

const netOf = (r: BaseRes) => Number(r.total_price || 0) - Number(r.commission || 0);

/** ✅ Règle d’éligibilité finale (identique à ta logique) */
...
/** évite des setState si les données n’ont pas réellement changé */
const rowsShallowEqual = (a: BaseRes[], b: BaseRes[]) => {
  if (a === b) return true;
  ...
  for (let i = 0; i < a.length; i++) {
    ...
    if (
      ...
      x.arrival_confirmation !== y.arrival_confirmation ||
      x.espece_confirmation !== y.espece_confirmation ||
      x.price_espece !== y.price_espece
    ) {
      return false;
    }
  }
        ...
        {values.map((v, i) => {
          ...
          return (
            ...
            >
              ...
              <Text style={[styles.barLabel, active && styles.barLabelActive]} numberOfLines={1}>
                {labels[i]}
              </Text>
            </TouchableOpacity>
          );
...
export default function EarningsScreen({ navigation }: any) {
  ...
  // —— anti-race : ignore les réponses anciennes
  const reqSeq = useRef(0);

  // —— debounce (RN/Web safe)
  ...
  const { dateMinISO, dateMaxISO, periodTitle } = useMemo(() => {
    ...
    return {
      ...
      periodTitle: cursorDay.toLocaleDateString("fr-FR", {
        day: "2-digit",
        month: "long",
        year: "numeric",
      }),
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // ======= Sélection (identique à ta logique) =======
      const baseSel = `
      ...
      const where = (q: any) =>
        q
          ...
          .or("price_espece.eq.0,espece_confirmation.is.true");
      ...
      const nextRows: BaseRes[] = res.flatMap((r) => (r.data as BaseRes[]) || []);

      // 🚀 évite re-render inutile + transition fluide
      if (ticket === reqSeq.current && !rowsShallowEqual(rows, nextRows)) {
        startTransition(() => {
          setRows(nextRows);
  ...
  }, [dateMinISO, dateMaxISO, kind, navigation, rows]);

  useEffect(() => {
    fetchData();
  }, [fetchData]);

  // realtime (réservations & transactions) — logique inchangée, juste debounced
  useEffect(() => {
    const ch = supabase
      .channel("earnings-rt")
      .on(
  ...
  // agrégation (identique)
  const currency = (rows[0]?.currency as string) || "XOF";
  const { labels, values, totals, counts } = useMemo(() => {
    ...
    if (gran === "year") {
      ...
      rs.forEach((r) => {
        ...
        vals[d.getUTCMonth()] += netOf(r);
        cnts[d.getUTCMonth()]++;
      });
      return {
        labels: monthNamesShort,
        values: vals,
        totals: { net: vals.reduce((a, b) => a + b, 0), commission: sumCommission },
        counts: cnts,
      };
    }
    if (gran === "month") {
  ...
  const goPrev = () => {
    if (gran === "year") setCursorYear((y) => y - 1);
      ...
      setCursorMonth((m) => (m === 0 ? 11 : m - 1));
      if (cursorMonth === 0) setCursorYear((y) => y - 1);
    } else {
  ...
  const goNext = () => {
    if (gran === "year") setCursorYear((y) => y + 1);
      ...
      setCursorMonth((m) => (m === 11 ? 0 : m + 1));
      if (cursorMonth === 11) setCursorYear((y) => y + 1);
    } else {
      const d = new Date(cursorDay);
      d.setDate(d.getDate() + 1);
      setCursorDay(d);
      ...
      <SafeAreaView edges={["top"]} style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 28 }} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.periodRow}>
            <TouchableOpacity onPress={goPrev} style={styles.pillNav}>
              <Ionicons name="chevron-back" size={18} color="#111" />
            </TouchableOpacity>
            <Text style={styles.periodTitle}>{periodTitle}</Text>
            <TouchableOpacity onPress={goNext} style={styles.pillNav}>
