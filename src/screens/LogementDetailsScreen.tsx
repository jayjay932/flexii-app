...
const supaThumb = (url?: string, w?: number, h?: number, q = 75, mode: "cover" | "contain" = "cover") => {
  if (!url) return undefined;
  try {
    if (url.includes("/storage/v1/object/public/")) {
      const [base] = url.split("?");
      const render = base.replace("/object/public/", "/render/image/public/");
      const pxW = Math.min(Math.round((w ?? SCREEN_W) * PixelRatio.get()), 1600);
      const pxH = Math.min(Math.round((h ?? BANNER_H) * PixelRatio.get()), 1600);
...
export default function LogementDetailsScreen({ route, navigation }: Props) {
  ...
  const addOnsTotals = useMemo(() => {
    return addOns.reduce((acc, addon) => {
      ...
      return acc + (addon.pricing_model === "per_night" ? addon.price * nights : addon.price);
    }, 0);
  ...
  const totalPrice = useMemo(() => {
    ...
    for (const addon of addOns) {
      if (selectedAddOns.includes(addon.id)) {
        base += addon.pricing_model === "per_night" ? addon.price * nights : addon.price;
      }
    }
    return base;
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      /* 3) Réservations -> indisponibles (fin EXCLUE) */
      const toUTC = (iso: string) => {
        const [y, m, d] = iso.split("-").map(Number);
        return new Date(Date.UTC(y, (m ?? 1) - 1, d ?? 1));
      };
      const keyUTC = (d: Date) => d.toISOString().slice(0, 10);
      const addRange = (startISO: string, endISO: string, into: Record<string, true>) => {
        try {
      ...
      (res ?? []).forEach((r) => {
        if (r.start_date && r.end_date) addRange(r.start_date, r.end_date, disabledByResa);
      });

      /* 3bis) Availability Overrides */
      ...
      const { data: addRows } = await supabase
        ...
        .select(`id, add_ons ( id, name, price, pricing_model )`)
        .eq("logement_id", id);
      ...
      if (Array.isArray(resRows) && resRows.length > 0) {
        ...
        revs =
          (rws ?? []).map((r: any) => ({
            ...
            reviewer: r.reviewer
              ? { id: r.reviewer.id, full_name: r.reviewer.full_name, avatar_url: r.reviewer.avatar_url }
              : undefined,
          })) ?? [];
      }
      ...
      const avg =
        reviews_count > 0
          ? Math.round((revs.reduce((s, rr) => s + (Number(rr.rating) || 0), 0) / reviews_count) * 10) / 10
          : undefined;

      // setState non bloquant pour éviter le jank
      startTransition(() => {
        setData({
          id: logement.id,
      ...
      // Préchargement images (12 premières)
      FastImage.preload(
        imgs.slice(0, 12).map((u) => ({
          uri: supaThumb(u, SCREEN_W, BANNER_H) ?? u,
  ...
  // Realtime — tables clés du listing, debounced
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`realtime-logement-${id}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "listings_logements", filter: `id=eq.${id}` },
        scheduleRefetch
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "listing_images", filter: `logement_id=eq.${id}` },
        scheduleRefetch
      )
      .on(
        "postgres_changes",
  ...
  const unitLbl = useMemo(() => unitLabel(data?.rental_type), [data?.rental_type]);
  ...
  /* ===== Day cell (mémo) ===== */
  const Day = React.memo(function DayCmp({
    date,
    state,
    marking,
    onPress,
    ...
    <View style={styles.root}>
      ...
      <View style={styles.banner}>
        <FlatList
          ...
          renderItem={({ item }) => {
            const uri = item ? supaThumb(item, SCREEN_W, BANNER_H, 80, "cover") : undefined;
            return (
              <FastImage
                source={uri ? { uri, priority: FastImage.priority.high } : PLACEHOLDER}
        ...
        <SafeAreaView edges={["top"]} style={styles.bannerTop}>
          ...
          <View style={{ flexDirection: "row", gap: 10 }}>
            ...
            <TouchableOpacity onPress={() => setFav((v) => !v)} style={styles.iconBtn}>
              <Ionicons name={fav ? "heart" : "heart-outline"} size={22} color={fav ? "#E61E4D" : "#000"} />
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      </View>
      ...
      <View style={styles.sheet}>
        <ScrollView contentContainerStyle={{ paddingBottom: 220 }} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.badgesLine}>
            {plural(data.max_guests, "voyageur", "voyageurs")} · {plural(data.bedrooms, "chambre", "chambres")} ·{" "}
            {plural(data.showers, "douche", "douches")}
            {data.toilets && data.toilets > 0 ? ` · ${plural(data.toilets, "toilette", "toilettes")}` : ""}
          </Text>
            ...
            <>
              ...
              <View style={styles.addOnWrap}>
                {addOns.map((opt) => {
                  ...
                  const priceLabel =
                    opt.pricing_model === "per_night" ? `${opt.price} XOF/nuit` : `${opt.price} XOF / séjour`;
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[styles.addOnItem, selected && styles.addOnItemSelected]}
                    >
                      <Ionicons
                      ...
                      <Text style={styles.addOnPrice}>
                        {opt.pricing_model === "per_night" ? `+${opt.price}XOF /nuit` : `+${opt.price}XOF`}
                      </Text>
                    </TouchableOpacity>
                  );
                })}
              ...
              >
                <View style={styles.hostCard}>
                  <View style={styles.hostRow}>
                    <FastImage
                      source={
                        data.owner.avatar_url
                          ? { uri: supaThumb(data.owner.avatar_url, 72, 72, 70, "cover") }
                          : PLACEHOLDER
                      }
                    ...
                    />
                    <View style={{ flex: 1 }}>
                      <Text style={styles.hostNameBig}>{data.owner.full_name}</Text>
                      <Text style={styles.hostRole}>Hôte</Text>
                    </View>
                    <View style={styles.hostStats}>
                      <Text style={styles.hostStatValue}>{data.reviews_count ?? 0}</Text>
                  ...
                  <View style={styles.hostInfoRow}>
                    ...
                    <Text style={styles.hostInfoText}>L’endroit où j’ai étudié : —</Text>
                  </View>
                  <View style={styles.hostInfoRow}>
                    <Ionicons name="language-outline" size={18} color="#222" />
                    <Text style={styles.hostInfoText}>Langues parlées : —</Text>
                  </View>

                  <View style={styles.hostDivider} />

                  <Text style={styles.hostInfoHeader}>Informations sur l’hôte</Text>
                  <Text style={styles.hostInfoText}>Taux de réponse : 100 %</Text>
                  <Text style={styles.hostInfoText}>Répond dans l’heure</Text>

                  <TouchableOpacity activeOpacity={0.9} style={styles.hostMessageBtn} onPress={onPressNegocier}>
                    <Text style={styles.hostMessageText}>Envoyer un message à l’hôte</Text>
                  </TouchableOpacity>
                </View>
              </TouchableOpacity>
            </>
          ...
          <View style={styles.reviewHero}>
            <Ionicons name="leaf" size={30} color="#333" style={{ opacity: 0.7 }} />
            <Text style={styles.reviewHeroScore}>
              {typeof data.rating === "number" ? data.rating.toFixed(2).replace(".", ",") : "4,84"}
            </Text>
            <Ionicons name="leaf" size={30} color="#333" style={{ opacity: 0.7 }} />
          </View>
          <Text style={styles.reviewHeroTitle}>Coup de cœur voyageurs</Text>
          <Text style={styles.reviewHeroDesc}>
            Ce logement fait partie des Coups de cœur voyageurs, à partir des évaluations, commentaires et de la
            fiabilité des annonces selon les voyageurs.
          </Text>

          {reviews.length === 0 ? (
            <Text style={{ color: "#666", marginTop: 8 }}>Aucun commentaire pour l’instant.</Text>
          ) : (
            <View style={styles.reviewGrid}>
              {reviews.slice(0, 2).map((rv) => {
                const isOpen = expanded[rv.id];
                const text = rv.comment ?? "";
                const short = text.length > 160 && !isOpen ? text.slice(0, 160).trimEnd() + "…" : text;
                return (
                  <View key={rv.id} style={styles.reviewTile}>
                    <View style={styles.reviewHeader}>
                      <FastImage
                        source={rv.reviewer?.avatar_url ? { uri: supaThumb(rv.reviewer.avatar_url, 42, 42, 70) } : PLACEHOLDER}
                        style={styles.reviewAvatar}
                        resizeMode={FastImage.resizeMode.cover}
                      />
                      <View style={{ flex: 1 }}>
                        <Text style={styles.reviewName}>{rv.reviewer?.full_name ?? "Voyageur"}</Text>
                        <Text style={styles.reviewDate}>{fmtDate(rv.created_at)}</Text>
                      </View>
                    ...
                    {text.length > 160 && (
                      <TouchableOpacity onPress={() => toggle(rv.id)}>
                        <Text style={styles.link}>{isOpen ? "Afficher moins" : "Afficher plus"}</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                );
          ...
          {reviews.length > 0 && (
            ...
            >
              <Text style={styles.showAllText}>Afficher les {reviews.length} commentaires</Text>
            </TouchableOpacity>
          )}
          ...
          <View style={styles.sectionBlock}>
            ...
            <Text style={styles.blockText}>
              Si vous annulez avant l'arrivée prévue, vous aurez droit à un remboursement partiel. Passé ce délai, cette
              réservation n'est pas remboursable.
            </Text>
            <View style={styles.inlineLinkRow}>
              <Text style={styles.inlineLink}>Consultez les conditions complètes de cet hôte</Text>
          ...
          <View style={styles.sectionBlock}>
            ...
            <Text style={styles.blockText}>Arrivée à partir de {fmtTime(data.check_in_start)}</Text>
            <Text style={styles.blockText}>Départ avant {fmtTime(data.check_out)}</Text>
            <Text style={styles.blockText}>{plural(data.max_guests, "voyageur maximum", "voyageurs maximum")}</Text>
        ...
        <View style={styles.bottomBar}>
          <View>
            <Text style={styles.price}>
              {totalPrice} <Text style={styles.priceUnit}>XOF/ {unitLbl} </Text>
            </Text>
            <Text style={styles.priceSmall}>
              {nights} nuit{nights > 1 ? "s" : ""} · {effectiveUnit}XOF /{unitLbl}
            </Text>
          </View>
        ...
        <View style={styles.calBackdrop}>
          <View style={styles.calSheet}>
            ...
            {tab === "dates" ? (
                ...
                {startDate && (
                  ...
                  >
                    {overridePrices[startDate] != null
                      ? `${overridePrices[startDate]} XOF / ${unitLbl}`
                      : `${data.price} XOF / ${unitLbl}`}
                  </Text>
                )}
              ...
              <View style={{ padding: 16 }}>
                <Text style={{ color: "#666" }}>
                  La vue “Dates” est active. “Mois” et “Flexible” sont décoratives.
                </Text>
              </View>
            )}
            ...
            <View style={styles.calFooter}>
              ...
              <TouchableOpacity
                style={[styles.nextBtn, (!startDate || savingReservation) && { opacity: 0.4 }]}
                disabled={!startDate || savingReservation}
                onPress={onValidateDates}
                activeOpacity={0.9}
              >
                {savingReservation ? <ActivityIndicator color="#fff" /> : <Text style={styles.nextBtnText}>Suivant</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  badgesLine: { marginTop: 6, fontSize: 15, color: "#6b6b6b", fontWeight: "600" },
  ...
  // Reviews hero
  reviewHero: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 12 },
  reviewHeroScore: { fontSize: 48, fontWeight: "900", color: "#111", letterSpacing: 0.5 },
  reviewHeroTitle: { textAlign: "center", marginTop: 8, fontSize: 20, fontWeight: "800", color: "#111" },
  reviewHeroDesc: { textAlign: "center", marginTop: 8, color: "#6b6b6b", lineHeight: 20 },

  // Reviews grid
  reviewGrid: { marginTop: 16, flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", gap: 12 },
  reviewTile: {
  ...
  link: { marginTop: 6, fontSize: 14, textDecorationLine: "underline", color: "#111", fontWeight: "700" },

  // Équipements
  equipWrap: { flexDirection: "row", flexWrap: "wrap", rowGap: 10, columnGap: 12, marginTop: 10 },
  equipItem: { flexDirection: "row", alignItems: "center", backgroundColor: "#fff", paddingVertical: 8, paddingHorizontal: 10, borderRadius: 12 },
  equipText: { marginLeft: 8, maxWidth: 160, fontSize: 14, fontWeight: "600", color: "#222" },
  ...
  hostMessageBtn: { marginTop: 14, backgroundColor: "#f3f3f3", borderRadius: 12, paddingVertical: 12, alignItems: "center" },
  hostMessageText: { fontWeight: "700", color: "#111" },

  // Bouton "Afficher les X commentaires"
  showAllBtn: { marginTop: 14, backgroundColor: "#f3f3f3", borderRadius: 12, paddingVertical: 12, alignItems: "center" },
  showAllText: { fontWeight: "800", color: "#111", fontSize: 15 },

  // Sections
  rowLink: { marginTop: 8, paddingVertical: 12, flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  rowLinkText: { fontSize: 15, fontWeight: "700", color: "#111" },

  sectionBlock: { marginTop: 12, paddingVertical: 12, borderTopWidth: 1, borderTopColor: "rgba(0,0,0,0.08)" },
  ...
  reportRow: { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 16, borderTopWidth: 1, borderTopColor: "rgba(0,0,0,0.08)" },
  ...
  calClose: {
    position: "absolute", left: 14, top: 14, zIndex: 2, width: 36, height: 36, borderRadius: 18,
